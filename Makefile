...
PROJECT_NAME_PARAM_STORE=dummy-paramter-store-project
PROJECT_NAME_PARAM_STORE_PROFILE=$(PROJECT_NAME_PARAM_STORE)-instance-profile
...
cfn-param-store-instance-profile-deploy: ## param store instance profiledeploy
	make internal-cfn-deploy CFN_PATH=deployments/cfn/example2/instance-profile.yml STACK_NAME=$(PROJECT_NAME_PARAM_STORE_PROFILE)

.PHONY: cfn-param-store-deploy
cfn-param-store-deploy: ## param store deploy
	make internal-cfn-deploy CFN_PATH=deployments/cfn/example2/template.yml STACK_NAME=$(PROJECT_NAME_PARAM_STORE)

.PHONY: cfn-param-store-all-delete
cfn-param-store-all-delete: ## param store all delete
	make internal-cfn-delete STACK_NAME=$(PROJECT_NAME_PARAM_STORE_PROFILE)
	make internal-cfn-delete STACK_NAME=$(PROJECT_NAME_PARAM_STORE)
